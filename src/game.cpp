...
void Game::Run(Controller const &controller, Renderer &renderer, std::size_t target_frame_duration) 
{
  ...
  // Launch Snake,
  // snake_thread = std::thread(&Game::RunSnake, this, controller, target_frame_duration);
...
void Game::RunSnake(Controller const& controller, std::size_t target_frame_duration)
{
    Uint32 frame_start;
    Uint32 frame_end;
    Uint32 frame_duration;
...
void Game::UpdateSnake() 
{
  ...
  if (food_pos.x == snake->head.x && food_pos.y == snake->head.y)     // Thread snake doesn't need to lock head.x & head.y
  {
